  ...
  run: async (client: TBotClient): Promise<void> => {
    try {
      const loadMessageFile = async (filePath: string, fileName: string) => {
        ...
        try {
          const command: ICommandFile = await import(filePath).then((module) => module.default);
          if (!command.name) {
          ...
          client.logStatus(`${fileName} (${command.name})`, "Message", ELogStatus.SUCCESS);
      ...
      const filteredMessages = messages.filter((file) => file.endsWith(`.${extension}`));
      await Promise.all(filteredMessages.map((file) => {
        const fileName = file.split(".")[0];
        return loadMessageFile(`../commands/${file}`, fileName);
      }));

      const subDirectories = messages.filter((file) => !file.endsWith(`.${extension}`));
      for (const subDirectory of subDirectories) {
        const subFiles = await readdir(`${__dirname}/commands/${subDirectory}`);
        const filteredSubFiles = subFiles.filter((file) => file.endsWith(`.${extension}`));
        await Promise.all(filteredSubFiles.map((file) => {
          const fileName = file.split(".")[0];
          return loadMessageFile(`../commands/${subDirectory}/${file}`, fileName);
        }));
      }
    } catch (error) {
